...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, default_filename=None, default_prefdef_class_file=None, default_save_dir=None):
        ...
        self.default_label_combo_box = DefaultLabelComboBox(self,items=self.label_hist,attribute='label')
        ...
        self.staff_security_combo_box = DefaultLabelComboBox(self,
                                    items=self.staff_security_items, attribute='staff_security')
        



        staff_security_qhbox_layout = QHBoxLayout()
        staff_security_qhbox_layout.addWidget(self.use_staff_security_checkbox)
        staff_security_qhbox_layout.addWidget(self.staff_security_combo_box)
        staff_security_container = QWidget()
        staff_security_container.setLayout(staff_security_qhbox_layout)
        ...
        self.gender_combo_box = DefaultLabelComboBox(self,items=self.gender_items, attribute='gender')

        gender_qhbox_layout = QHBoxLayout()
        gender_qhbox_layout.addWidget(self.use_gender_checkbox)
        gender_qhbox_layout.addWidget(self.gender_combo_box)
        gender_container = QWidget()
        gender_container.setLayout(gender_qhbox_layout)
        ...
        self.prev_age = None
        self.age_combo_box = DefaultLabelComboBox(self,items=self.age_items, attribute='age')
        # [Trẻ em - Thanh thiếu niên - Thanh niên - Trung niên - Người lớn tuổi]

        age_qhbox_layout = QHBoxLayout()
        age_qhbox_layout.addWidget(self.use_age_checkbox)
        age_qhbox_layout.addWidget(self.age_combo_box)
        age_container = QWidget()
        age_container.setLayout(age_qhbox_layout)
        ...
        self.prev_QA = None
        self.QA_combo_box = DefaultLabelComboBox(self,items=self.QA_items, attribute='QA')

        QA_qhbox_layout = QHBoxLayout()
        QA_qhbox_layout.addWidget(self.use_QA_checkbox)
        QA_qhbox_layout.addWidget(self.QA_combo_box)
        ...
        list_layout.addWidget(gender_container)
        list_layout.addWidget(age_container)
        list_layout.addWidget(QA_container)
                # **** Thinkman custom labelImg ****
    ...
    def edit_label(self):
        ...
        if text is not None:
            ...
            item.setBackground(generate_color_by_text(self.items_to_shapes[item].get_string()))
            self.set_dirty()
    ...
    def shape_selection_changed(self, selected=False):
        if self._no_selection_slot:
            ...
            if shape:
                ...
                # nếu là shape thì ta cập nhật các thanh attribute
    ...
    def add_label(self, shape):
        ...
        item = HashableQListWidgetItem(shape.get_string())
        item.setFlags(item.flags() | Qt.ItemIsUserCheckable)
        item.setCheckState(Qt.Checked)
        # item.setBackground(generate_color_by_text(shape.label))
        item.setBackground(generate_color_by_text(shape.get_string()))
    ...
    def load_labels(self, shapes):
        ...
        for label, info, points, line_color, fill_color, difficult, direction in shapes:
            shape = Shape(label=label, staff_security=info[0], gender=info[1], age=info[2], QA = info[3])
            for x, y in points:
            ...
            if line_color:
                ...
                shape.line_color = generate_color_by_text(shape.get_string())

            if fill_color:
                shape.fill_color = QColor(*fill_color)
            else:
                # shape.fill_color = generate_color_by_text(label)
                shape.fill_color = generate_color_by_text(shape.get_string())
    ...
    def save_labels(self, annotation_file_path):
        ...
        # tạo ra info list để biết index của detection này
        info_items_list = [self.staff_security_items, self.gender_items, self.age_items, self.QA_items]

        def format_shape(s):
            return dict(label=s.label,
                        # thêm info cho detection
                        info = [s.staff_security, s.gender, s.age, s.QA],
        ...
        # thinkman edit here
        # chỗ này ta tạm thời set cứng lưu theo YOLO format
        self.label_file_format = LabelFileFormat.YOLO
        ...
        try:
            if self.label_file_format == LabelFileFormat.PASCAL_VOC:
                ...
                self.label_file.save_yolo_format(annotation_file_path, shapes, self.file_path, self.image_data, self.label_hist,
                                                 self.line_color.getRgb(), self.fill_color.getRgb(),info_items_list = info_items_list)
            elif self.label_file_format == LabelFileFormat.CREATE_ML:
    ...
    def staff_security_combo_selection_changed(self, index):
        # check xem item hiện tại có phải là ảnh hay không
        item = self.current_item()
        if item in self.items_to_shapes.keys():
            shape = self.items_to_shapes[item]
            shape.line_color = generate_color_by_text(shape.get_string())
            self.canvas.select_shape(shape)
            shape.staff_security = self.staff_security_items[index]
    ...
    def new_shape(self):
        ...
        # cái này là cốc gốc chỗ phần dùng label cho người, ta sẽ không đụng vào chỗ này
        ...
        # ta làm lại y chang
        if self.prev_staff_security is not None:
            staff_security = self.prev_staff_security
        else:
            staff_security = self.default_staff_security
            self.prev_staff_security = staff_security
        if self.prev_gender is not None:
            gender = self.prev_gender
        else:
        ...
        if text is not None:
            ...
            generate_color = generate_color_by_text(f"{staff_security}-{gender}-{age}-{QA}")
            shape = self.canvas.set_last_label(text, staff_security, gender, age, QA,
    ...
    def load_yolo_txt_by_filename(self, txt_path):
        ...
        t_yolo_parse_reader = YoloReader(txt_path, self.image, info_items_list = info_items_list)
    ...
    def check_txt_label_type(self, txt_path):
        with open(txt_path, 'r') as f:
            ...
            # thinkman edit here để dùng trong tương lai
            # hiện tại nếu chứa 9 số thì là yolo
